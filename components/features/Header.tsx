...
export default function Header() {
  ...
  // Header is solid if scrolled, not on a dark hero page, OR if menu is open
  ...
  return (
    <>
      ...
      >
        <Container>
          <Flex as="nav" align="center" justify="between">
            ...
            {/* Mobile hamburger — always visible, toggles menu */}
      ...
      {/* ── Mobile menu overlay ────────────────────────────────────────
          FRÈRE du header (pas enfant) → contexte de stack indépendant
          z-[60] > z-50 du header → couvre toute la page sans exception
      ─────────────────────────────────────────────────────────────── */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            className="md:hidden fixed inset-0 z-[60] bg-[#1a2414] flex flex-col"
          ...
          >
            ...
            >
              <Link
                href={isLoggedIn ? (isPro ? "/dashboard" : "/account") : "/auth/select-space"}
                onClick={() => setMobileMenuOpen(false)}
              >
