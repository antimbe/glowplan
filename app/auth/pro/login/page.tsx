...
import { ArrowLeft, Briefcase, Loader2, MailWarning, Send } from "lucide-react";
    ...
    try {
      ...
      if (isLogin) {
        ...
        if (userType === 'client') {
          ...
          throw new Error("Ce compte est un compte client. Vous semblez vous connecter au mauvais endroit. Veuillez utiliser l'espace client.");
        }

        // Vérification supplémentaire via les tables
        ...
        if (!establishment && userType !== 'pro') {
          // Si pas d'établissement et pas marqué pro, vérifier si profil client existe
        ...
        <form onSubmit={handleAuth} className="w-full">
          <Stack space={6} align="center" className="w-full">
            ...
            <Button
              ...
              fullWidth
              disabled={loading}
              className="bg-primary hover:bg-primary-dark rounded-xl font-bold h-14 text-sm shadow-2xl shadow-primary/20 transition-all duration-500 hover:scale-[1.02] active:scale-[0.98] mt-2 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {loading ? (
        ...
        <Box className="pt-6 border-t border-gray-100 w-full">
          <Flex justify="center" align="center" gap={3}>
            ...
            <Button
              ...
              className="text-primary hover:text-primary-dark font-bold text-sm underline underline-offset-4 p-0 h-auto min-w-0"
            >
        ...
        <Stack space={8} align="center" className="w-full max-w-lg mx-auto">
          ...
          >
            <Link
              ...
              className="group inline-flex items-center gap-2.5 text-white/50 hover:text-white transition-all mb-8 px-4 py-2 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/10 backdrop-blur-sm"
            >
