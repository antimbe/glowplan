  ...
  try {
    ...
    return NextResponse.json({
      ...
      upcomingAppointments: upcomingApts?.map(apt => ({
        ...
        service: Array.isArray(apt.services) ? apt.services[0]?.name : apt.services?.name,
        provider: Array.isArray(apt.establishments) ? apt.establishments[0]?.name : apt.establishments?.name,
